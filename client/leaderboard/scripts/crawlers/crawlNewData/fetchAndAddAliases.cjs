...
const eliminateDuplicates = (aliasArray) => {
  ...
  Object.keys(addressToIndicesMapping).forEach((address) => {
    ...
    if (indices.length === 1) {
      ...
      const entryWithLastCreatedDate = getEntryWithLastCreatedDate(
        indices.map((index) => aliasArray[index])
      );
...
const getEntryWithLastCreatedDate = (aliasArray) => {
  ...
  for (let i = 1; i < aliasArray.length; i++) {
    if (
      Date.parse(aliasArray[i].createdate) >
      Date.parse(entryWithLastCreatedDate.createdate)
    ) {
